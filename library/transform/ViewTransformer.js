...
export default class ViewTransformer extends React.Component {
    ...
    performDoubleTapUp(pivotX, pivotY) {
        ...
        const scaleTo = this.props.doubleTapScaleTo || this.props.maxScale;
        let curScale = this.state.scale;
        let scaleBy;
        // if current scale is more then half way to scaleTo go back to 1
        if (curScale > (1 + scaleTo) / 2) {
            scaleBy = 1 / curScale;
        }
        // else to to scaleTo
        else {
            scaleBy = scaleTo / curScale;
        }
...
ViewTransformer.propTypes = {
     ...
     * Default to maxScale
     */
    doubleTapScaleTo: React.PropTypes.number,
